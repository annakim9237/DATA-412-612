---
title: "How COVID affected the Economy"
...
This code chunk is for cleaning COVID case.
...
This chunk is for cleaning COVID case per state
```{r, warning=FALSE, message=FALSE}
...
This chunk is to make the states categories
```{r, warning=FALSE, message=FALSE}
...
This chunk is to calculate the formula
```{r, warning=FALSE, message=FALSE}
...
This chunk is to join every data
```{r, warning=FALSE, message=FALSE}
...
This chunk is to read final version data set
...
```
This line graph shows the unemployment rate in each U.S. state from 2017 to 2022.
...
Interestingly, while credit card and car-related debt increased,
other types of debt like student loans and mortgages actually went down.

This suggests that government programs or debt relief policies may have helped reduce those types of loans.
But personal spending and borrowing still increased for many individuals.
...
**Left chart:**
- The left shape (red) shows credit card debt changes before COVID.

- The right shape (blue) shows the changes after COVID. The shape is higher and more spread out after COVID, which means credit card debt increased more in many states.

**Right chart:**
- This chart shows how often people couldn’t pay back their credit card bills on time.

- The shapes are very similar before and after COVID, and even slightly lower after.
...
**Left chart:**
The before COVID is taller and wider, meaning more states had higher increases in student debt.

The after COVID is lower and tighter, showing that student loan debt grew more slowly and in some cases even decreased.

**Right chart:**
The before COVID shows high and variable delinquency rates.
...
**Auto Loan (Two Left)**
The green box (post) is higher and wider than the red box (pre). This means people borrowed more for cars after the pandemic.

**Auto Loan Delinquency (Two Right)**
The purple and blue boxes are similar, but the purple one (post) is slightly lower. This means that even though borrowing increased, people still paid their car loans on time.
...
# Housing Change Rate Visualizations (New) {.tabset}
...
Before COVID, housing prices were mainly influenced by population and mortgage.
After COVID, broader economic factors such as GDP, total debt, and credit card borrowing had a stronger impact.
...
Population (ignificant): More people → Higher house price change

Mortgage delinquency (ignificant): More mortgage trouble → Lower prices
